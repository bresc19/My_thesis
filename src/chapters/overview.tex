...
K-Fold cross-validation consist in dividing a data set into k multiple training and validations sets (folds), to improve model results against the random selection of only one training and validation set. Indeed, errors and accuracy evaluation is averaged along the k different random sample.
Metrics chosen for evaluation are MAE (Mean Absolute Error), MSE (Mean Squared Error) and R2 (Coefficient of determination).
