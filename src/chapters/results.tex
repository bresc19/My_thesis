...
\begin{comment}
For each test, the training phase was performed with the 2 set of feature configured in the previous subsection: one with the features selected for each specific data set period and one which is the
For doing that 2 ML model were implemented:
\end{comment}
\begin{itemize}
    \item Neural Network with Keras;
