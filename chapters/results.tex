...
For instance, DTM average elevation and slope ('h\_mean' and 'slope\_mean' respectively) are correlated in the same way in test of both target variables, since air pollution affect more urban than mountain areas.
...
It can be shown by figure \ref{fig:pearson_general} that the entire set of pollutants variables are almost all positively correlated to it.
In particular PM10 provided by ARPA and CAMS, in the various test executed is one of the most correlated pollutant. Indeed, 'pm10\_int' and 'pm10\_cams' have always an high number of vote. Strong positive correlation between PM2.5 and PM10 is also proved in literature\cite{zhou2016concentrations}.
Another variable which received a strong number of votes is the PM2.5 modelled by CAMS ('pm25\_cams'). 
Both variables in fact measures the same pollution phenomena as the target variables. This finding make us understand how both describe the same pollution phenomena, moving to the same direction. Indeed, correlation coefficient between them is near to 1 in all test executed.
...
From nitrogen dioxide derives also the chemical relation with the other nitrogen oxides ('nox\_int'), such as nitrogen monoxide ('no\_int') and carbon monoxide ('co\_int', 'co\_cams, 'co\_s5p'). This could be explain by the fact that they contribute as secondary pollutant to PM2.5 formation \cite{xie2015spatiotemporal}.
\\
...
So we can assume that intense agriculture is one of the factor that influence the PM2.5 formation, with greater contribution in a certain period of the year.\\
\pagebreak
\clearpage
\begin{figure}[H]
\centering
...
In the results obtained along the 5 period, stands out the ammonia provided by CAMS model ('nh3\_cams') which, due to the fact that is strongly correlated, represents the same pollutant as well.
...
It's observable also the votes received by the variable that models the soil moisture, always with a positive correlation with respect to the target variable. This could be explained by the abundance of ammonia-oxidizing bacteria which increase as the soil moisture \cite{avrahami2007response}.  
