...
The k chosen used to increase the number of cells with a value is given by a trade-off between dimension of k and both the accuracy and processing time that allows us to apply FS.
...
In the figure TODO is shown the processing time taken to run this method for resolution at 1 km in function of the k choosen.
For 1 km resolution I chose a k values which allows me to have  an minimal increment in relation to the space covered by each cell. Indeed, using a K proportional the time spent for running the algorthms to the tends to be too high.    
In addition, the values of k for each resolution chosen bring 2 different spatial extensions that could be seen in figure hhh.
