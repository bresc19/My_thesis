...
Data covariates are divided between input (X) and output variable (Y). X represents all the variables collected in the previous part, except for the pollutant to be analyzed and modeled (such as PM25 or Ammonia) which is assigned to the Y variable.
...
Then, these cells are set with values computed using a \gls{rbf} \cite{wright2003radial} which are separately stored in variables with name that ends with '\_int'. RBF was performed using a linear interpolation by a colleague.
